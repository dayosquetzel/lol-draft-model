...
def _ensure_csv_downloaded(path: Path, url: str, min_expected_bytes: int = 1_000_000) -> str:
    ...
    try:
        ...
        size = path.stat().st_size
        return f"Downloaded {path.name} ({size:,} bytes)"
    except Exception as e:
        return f"FAILED to download {path.name}: {type(e).__name__}: {e}"

_CSV_DOWNLOAD_STATUS = _ensure_csv_downloaded(_CSV_2026, _CSV_2026_URL)
...
def _data_banner():
    ...
    st.caption(f"⬇️ {_CSV_DOWNLOAD_STATUS}")
